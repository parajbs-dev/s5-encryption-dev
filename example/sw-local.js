...
async function getStreamingLocation(hash, types) {
  ...
  if (PORTAL_URL.startsWith("http://")) {
    localtionApiUrl = PORTAL_URL + "/s5/debug/storage_locations/";
  }

  if (PORTAL_URL.startsWith("https://localhost")) {
    localtionApiUrl = PORTAL_URL + "/s5/debug/storage_locations/";
  } else {
    if (PORTAL_URL.startsWith("https://")) {
      localtionApiUrl = "https://s5.cx/api/locations/";
    }
  }

  // TODO Expiry

  console.debug('fetch', localtionApiUrl + hash + '?types=' + types)
  const res = await fetch(localtionApiUrl + hash + '?types=' + types);
        ...
        if (chunkRes !== undefined) {
          ...
          if (downloadingChunkLock[chunkLockKey] === true && !lockedChunks.includes(chunkLockKey)) {
            ...
            while (downloadingChunkLock[chunkLockKey] === true) {
              ...
              downloadingChunkLock[chunkLockKey] = false;
            }
