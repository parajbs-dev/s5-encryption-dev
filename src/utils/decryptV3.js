 ...
 */
async function decrypt_file_xchacha20(encryptedFile, key, padding, chunkIndex) {
  await sodium.ready;
...
try {
  ...
  for (let i = 0; i < readerLength; i += chunkSize) {
    ...
    decryptedData.push(ciphertext);
    console.log('decryptIndex: ', chunkIndexIntern);
    chunkIndexIntern++;
  }

  const totalLength = decryptedData.reduce((acc, chunk) => acc + chunk.length, 0);
  const decryptedFileBytes = new Uint8Array(totalLength);

  let offset = 0;

  for (const chunk of decryptedData) {
    decryptedFileBytes.set(chunk, offset);
    offset += chunk.length;
  }
  
  return await decryptedFileBytes;
  } catch (error) {
 ...
 */
async function decryptData(encryptedFile, decryptedFile, decryptionKey) {
  ...
  const data = chunks.reduce((accumulator, currentChunk) => {
    return Buffer.concat([accumulator, currentChunk]);
  }, Buffer.alloc(0));
  ...
  function writeNextChunk() {
    ...
    if (offset < decryptedFileBytes.length) {
      ...
      chunkIndex++;
      console.log('WriteIndex:  '+chunkIndex)
    } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    decryptedFileStream.on("finish", () => resolve("File write completed."));
